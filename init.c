...
void init_torus()
{
    ...
    //gam = 5./3.; //
    gam = 13./9.; // relativistic electrons; nonrel. ions
    ...
    cpow2  = 1.0; //exponent prefactor (the larger it is, the more hyperexponentiation there is)
    ...
    global_x10 = 3.0;  //radial distance in MCOORD until which the innermost angular cell is cylindrical
    global_x20 = -1. + 1./mpi_ntot[2];     //This restricts grid cylindrification to the one
    //single grid cell closest to the pole (other cells virtually unaffected, so their evolution is accurate).
    ...
    DTd = 10.; // dumping frequency, in units of M 10 (dumpXXX)
    DTl = 10.; // logfile frequency, in units of M 10
    DTi = 10.; // image file frequ., in units of M 10
    DTr = 10.; // restart file frequ., in units of M 10 (rdumpXXX)
    DTr01 = 100.; // restart file frequ., in timesteps 100 (rdump0, rdump1)
    ...
    for(iglob = 0; iglob < mpi_ntot[1]; iglob++) {
        for(jglob = 0; jglob < mpi_ntot[2]; jglob++) {
            for(kglob = 0; kglob < mpi_ntot[3]; kglob++) {
                ...
                AAin = (rin*rin + a*a)*(rin*rin + a*a)- DDin*a*a*sthin*sthin;
                ...
                // so it needs to be transformed at the end
                    ...
                    // convert from 4-vel in BL coords to relative 4-vel
                    // in code coords
    ...
    if (WHICHFIELD == NORMALFIELD) {
      //aphipow = 0.; // single loop
        aphipow = 0.75; // multi-loop 0.75
    }
    else {
    ...
    ZSLOOP(0,N1-1+D1,0,N2-1+D2,0,N3-1+D3) {
        ...
        */
        ...
        // MULTI-LOOP
        if (WHICHFIELD == NORMALFIELD) {
            q = (rho_av/rhomax - 0.2)*pow(r, aphipow);
        }

        double SSS = 1.1*rin;
        double lambdafieldratio = 0.09; // default 0.065
        double lambdafield = lambdafieldratio*2.0*M_PI*r;
//        double lambdafield = 0.09; // default 0.065
//        double lambdafieldratio = lambdafield/(2.0*M_PI*r);

        if (q > 0.) {
            A[i][j][k] = pow(q, 2.) * sin((log10(r/SSS))/lambdafieldratio);// * (1 + 0.02*(-0.5));
        }
        // END MULTI LOOP
