...
{
  double Fcon_calc(double *prim, int i1, int i2, int i, int j, int k);
  int i,j,k,m ;
  double divb ;
  double X[NDIM] ;
  ...
  // calculate current (note that this is only done when dumping)
  current_calc();

  //static double a_pa[N1M][N2M][N3M][NPR];
  //static double (* pa)[N2M][N3M][NPR];
  //pa = (double (*) [N2M][N3M][NPR])(&( a_pa[N1G][N2G][N3G][0])) ;
  //static double pa[N1M][N2M][N3M][NPR];
  /*
  double gF0p[NDIM], gF0m[NDIM], gF1p[NDIM];
  ZSLOOP(-1, N1-1+N1G, -1, N2-1+N2G, -1, N3-1+N3G) PLOOP pa[i][j][k][m] = 0.5*(p[i][j][k][m] + psave[i][j][k][m]);
  ZLOOP for(m = 0; m < NDIM; m++) Jcon[i][j][k][m] = 0.;
  ZLOOP {
      for(m = 0; m < NDIM; m++)
  ...
  */
  ...
  ZSLOOP(0-WG*N1G,N1-1+WG*N1G,0-WG*N2G,N2-1+WG*N2G,0-WG*N3G,N3-1+WG*N3G) {
    ...
    if(!failed) {
      ...
      for (m = 0; m < NDIM; m++)
          n += write_to_dump(is_dry_run, fp, dump_buffer+n, Jcon[i][j][k][m]);

      double jcov[NDIM];
      lower(Jcon[i][j][k], &geom, jcov);
      //lower(Jcon, geom, jcov);
      for (m = 0; m < NDIM; m++)
          n += write_to_dump(is_dry_run, fp, dump_buffer+n, jcov[m]);
